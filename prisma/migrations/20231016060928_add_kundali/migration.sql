...
-- DropIndex
DROP INDEX `ChatSession_astrologer_id_fkey` ON `chatsession`;

-- DropIndex
DROP INDEX `ChatSession_conversation_id_fkey` ON `chatsession`;

-- DropIndex
DROP INDEX `ChatSession_user_id_fkey` ON `chatsession`;
...
-- AddForeignKey
ALTER TABLE `ChatSession` ADD CONSTRAINT `ChatSession_astrologer_id_fkey` FOREIGN KEY (`astrologer_id`) REFERENCES `Astrologer`(`ID`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Kundali` ADD CONSTRAINT `Kundali_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `User`(`ID`) ON DELETE RESTRICT ON UPDATE CASCADE;
