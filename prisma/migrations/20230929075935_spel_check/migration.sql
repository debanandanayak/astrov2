/*
  ...
  - The values [astroloer] on the enum `Blog_Post_created_by` will be removed. If these variants are still used in the database, this will fail.
  - The values [astroloer] on the enum `Message_sender` will be removed. If these variants are still used in the database, this will fail.
...
-- AlterTable
ALTER TABLE `blog_post` MODIFY `created_by` ENUM('user', 'astrologer', 'admin') NOT NULL;

-- AlterTable
ALTER TABLE `message` MODIFY `sender` ENUM('user', 'astrologer') NOT NULL;
